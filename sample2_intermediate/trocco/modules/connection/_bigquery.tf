...
resource "trocco_connection" "bigquery_sample_project" {
  ...
  name                     = "${var.google.sample_project_id}__trocco@${var.google.sample_project_id}.iam.gserviceaccount.com"
  ...
  service_account_json_key = "{\"project_id\":  \"${var.google.sample_project_id}\", \"info\": \"dummy json key: please replace after creation for not to ingest credential into state file\"}"
}
...
resource "trocco_connection" "bigquery_sample_project2" {
  connection_type          = "bigquery"
  name                     = "${var.google.sample_project_id}2__trocco@${var.google.sample_project_id}2.iam.gserviceaccount.com"
  ...
  service_account_json_key = "{\"project_id\":  \"${var.google.sample_project_id}2\", \"info\": \"dummy json key: please replace after creation for not to ingest credential into state file\"}"
}
